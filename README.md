...
* Retrieve Metal network information from a node

### Requirements

MetalJS requires Node.js LTS version 14.16.0 or higher to compile.

### Installation

Metal is available for install via `yarn`:
...
This will generate a pure Javascript library and place it in a folder named "web" in the project root. The "metal.js" file can then be dropped into any project as a pure javascript implementation of MetalJS.

The MetalJS library can be imported into your existing Node.js project as follows:
...
```js
import { Metal, BinTools, BN, Buffer } from "@metalblockchain/metaljs"
...
* Metal: Our javascript module.
* BinTools: A singleton built into MetalJS that is used for dealing with binary data.
* [BN](https://www.npmjs.com/package/bn.js): A bignumber module use by MetalJS.
* [Buffer](https://www.npmjs.com/package/buffer): A Buffer library.

## Example 1 &mdash; Managing X-Chain Keys

MetalJS comes with its own AVM Keychain. This KeyChain is used in the functions of the API, enabling them to sign using keys it's registered. The first step in this process is to create an instance of MetalJS connected to our Metal Platform endpoint of choice.

```js
import { Metal, BinTools, Buffer, BN } from "@metalblockchain/metaljs"

const bintools = BinTools.getInstance()

const myNetworkID = 12345 //default is 1, we want to override that for our local network
const metal = new Metal("localhost", 9650, "http", myNetworkID)
const xchain = metal.XChain() //returns a reference to the X-Chain used by MetalJS
```
...
This example creates an asset in the X-Chain and publishes it to the Metal Platform. The first step in this process is to create an instance of MetalJS connected to our Metal Platform endpoint of choice.

```js
import { Metal, BinTools, Buffer, BN } from "@metalblockchain/metaljs"
import { InitialStates, SECPTransferOutput } from "@metalblockchain/metaljs/dist/apis/avm"
...
The first steps in creating a new asset using MetalJS is to determine the qualities of the asset. We will give the asset a name, a ticker symbol, as well as a denomination.
...
Using the MetalJS X-Chain API, we going to call the `issueTx` function. This function can take either the Tx class returned in the previous step, a CB58 representation of the transaction, or a raw Buffer class with the data for the transaction. Examples of each are below:
...
This example sends an asset in the X-Chain to a single recipient. The first step in this process is to create an instance of Metal connected to our Metal Platform endpoint of choice.

```js
import { Metal, BinTools, Buffer, BN } from "@metalblockchain/metaljs"

const myNetworkID = 12345 // default is 1, we want to override that for our local network
const metal = new metal.Metal(
  "localhost",
  9650,
  "http",
  myNetworkID
